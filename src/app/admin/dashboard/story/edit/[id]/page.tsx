...
export default function EditStoryPage() {
  ...
  useEffect(() => {
    const fetchStory = async () => {
      try {
        ...
        let parsedImages = [];
        try {
          const firstParse = JSON.parse(data.images || "[]");
        ...
        let parsedTags = [];
        try {
          const firstParse = JSON.parse(data.tags || "[]");
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="mb-8">
        <h2 className="text-2xl font-bold tracking-tight">스토리 수정</h2>
        <p className="text-muted-foreground mt-1">블로그 형식의 소식과 스토리를 수정합니다.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white p-8 rounded-lg shadow border border-gray-200">
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-900 flex justify-between">
            ...
            <span className="text-xs text-gray-500 font-normal">HTML 지원</span>
          </label>
          <textarea 
            value={content}
        ...
        <div className="flex flex-col gap-4 pt-4">
          <div className="flex items-center justify-between p-4 bg-primary/5 rounded-lg border border-primary/20">
            <div>
              <h4 className="font-semibold text-primary mb-1 text-sm">✨ AI 에디터 렌더링 사용</h4>
              <p className="text-xs text-gray-500">내용에 적힌 지시사항을 바탕으로 감성적인 HTML 구조를 처음부터 새로 생성합니다. (기존 내용 덮어쓰기)</p>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="isVisible" className="text-sm font-medium text-gray-700">공개하기</label>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-gray-200 flex justify-end gap-4">
          ...
          >
            {isSubmitting ? (useAI ? "✨ AI 렌더링 중..." : "저장 중...") : "스토리 수정"}
          </button>
        </div>
      </form>
    </div>
