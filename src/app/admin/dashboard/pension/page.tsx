...
export default function PensionManagementPage() {
  ...
  const handleUpdate = async (section: string, key: string, value: string) => {
    ...
    try {
      await fetch("/api/admin/content", {
        ...
        body: JSON.stringify({ page: "pension", section, key, value, type: "text" }),
      });
    } catch (err) {
  ...
  if (loading) return <div className="animate-pulse">Loading Pension Management...</div>;

  return (
    <div className="space-y-12">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold text-[#171212] tracking-tight">Pension Management</h1>
          <p className="text-[#856669] mt-2 font-medium">Manage the details and imagery for the Accommodation section.</p>
        </div>
        ...
        <section className="bg-white rounded-3xl border border-[#e4dcdd] overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-[#856669]">Page Hero Title</label>
              <input 
                type="text" 
                defaultValue={content.find(c => c.section === "hero" && c.key === "title")?.value || "Accommodation"}
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-[#856669]">Main Description</label>
              <textarea 
                rows={4}
                defaultValue={content.find(c => c.section === "about" && c.key === "description")?.value || "Thoughtfully designed rooms that frame the surrounding landscape."}
        ...
        <section className="bg-white rounded-3xl border border-[#e4dcdd] overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            <div className="space-y-2">
              ...
              <input 
                type="text" 
                defaultValue={content.find(c => c.section === "hero" && c.key === "imageUrl")?.value || "/images/lovable/pension.jpg"}
